...
* Active Monitor
  ...
  * Electron 开发环境下运行几次，cpu 资源占用就非常高，主要进程是 `coreservicesd` 和 `mds`

    * 跟 Electron 没什么关系，不定时的出现这个问题，尝试关闭 Spotlight 的索引^（sudo mdutil -a -i off）^并删除对应数据^（sudo rm -rf /System/Volumes/Data/.Spotlight-V100/*）^试试
